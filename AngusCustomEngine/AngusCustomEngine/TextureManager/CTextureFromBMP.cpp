...
DWORD WINAPI CTexture_Load_BMP_From_Disk( PVOID pvParam )
{
	...
	CTextureFromBMP* pTextureFromBMP = (CTextureFromBMP*)pvParam;

	//std::cout << "Thread: is loading " << pTextureFromBMP->getFileNameFullPath() << std::endl;
	//std::cout.flush();

	pTextureFromBMP->CreateNewTextureFromBMPFile_LoadFromDisk( pTextureFromBMP->getFileNameFullPath() );		

	//std::cout << "Thread: "<< pTextureFromBMP->getFileNameFullPath() << " is loaded" << std::endl;
	//std::cout.flush();
...
bool CTextureFromBMP::LoadBMP2_Async( std::string fileNameFullPath )
{
	// Create thread
	LPDWORD phThread = 0;	
	DWORD hThread = 0;	
	...
	hThreadHandle = CreateThread(NULL,	// Default security
								 ...
								 &CTexture_Load_BMP_From_Disk, // Pointer to the thread's function
...
{
	  // For asyn loading
	this->textureLoadStatus = UNKNOWN;
...
// **************************************************************
//      _   _____   ___  _  ___   _    ___   _   ___  
//     /_\ / __\ \ / / \| |/ __| | |  / _ \ /_\ |   \ 
//    / _ \\__ \\ V /| .` | (__  | |_| (_) / _ \| |) |
//   /_/ \_\___/ |_| |_|\_|\___| |____\___/_/ \_\___/ 
//                                                    
// This ONLY loads the data from disk into the internal array
bool CTextureFromBMP::CreateNewTextureFromBMPFile_LoadFromDisk( 
	std::string fileNameFullPath )
{
	...
	// File is loaded and the internal array has the image in it
	this->textureLoadStatus = CTextureFromBMP::IS_READY_TO_BE_COPIED_TO_GPU;

	return true;
}

// This will take an existing, loaded array and place it into the GPU
// WARNING: will crash if the array doesn't have anything in it, 
// or if called from another thread (not the main rendering thread)
bool CTextureFromBMP::CreateNewTextureFromBMPFile_CopyToGPU( bool bGenerateMIPMap )
{
	//std::cout << "Copying " << this->getFileNameFullPath() << " to GPU..." << std::endl;
	//std::cout.flush();
...
//					 this->m_p_theImages );
	...
	this->textureLoadStatus = CTextureFromBMP::IS_IN_GPU;
